...
export class InscripcionesController {
    ...
    async buscar(@Param('id') id: string) {
        const inscripcion = await this.servicio.buscarPorId(Number(id));
        if (!inscripcion) {
...
async crear(
  ...
  // Validacion minima a mano. 400 = no entiendo la peticion.
  // 409 = la entiendo, pero choca con el estado actual del sistema.
